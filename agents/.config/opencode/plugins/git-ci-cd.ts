...
export const gitCiCd: Plugin = async ({ $, directory }) => {
  return {
    "tool.execute.before": async (input, output) => {
      ...
      const workflowsExist = await $`test -d ${workflowsPath}`
        .then(() => true)
        .catch(() => false);
      ...
      if (!ghExists) {
        const msg =
          "GitHub Actions found but gh CLI not installed. Check with user if CI/CD passed before continuing developing.";
        await client.app.log({ body: { level: "warn", message: msg } });
        throw new Error(msg);
      }

      const runResult =
        await $`gh run list --branch ${pushedBranch} -L 1 --json status,conclusion`.catch(
          () => null,
        );
      ...
      try {
        ...
        if (
          ...
          const msg =
            "GitHub Actions failed for latest run. Check with user if CI/CD passed before continuing developing.";
          await client.app.log({ body: { level: "warn", message: msg } });
          throw new Error(msg);
        }
      } catch {
